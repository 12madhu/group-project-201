...
library("shinythemes")
song.data <- read.csv("../group-project-201/featuresdf.csv", stringsAsFactors = FALSE)
...
# Graph of specified features
linear.model <- ggplot(data = songs, mapping = aes(x = rank, y = tempo, color = duration_ms, size = instrumentalness)) +
  geom_point() +
